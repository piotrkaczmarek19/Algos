...
def train(x,Y,W, test_set, alpha = 0.1, iterations=10000):
	costs = []
	...
	Y_test = encode_labels( test_set[1])
	accuracy = np.sum(np.rint(np.logical_and((test_set[0].dot(W.transpose())),Y_test))) / len(test_set[1]) * 100
